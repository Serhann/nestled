 ...
 */
const LiveView = lazy(() =>
  import('./LiveView').then((m) => ({ default: m.LiveView })),
);
 ...
 * actually watching — buffering everyone continuously is how a single process
 * runs out of memory — so the `watch` control frame is what turns the recording
 * on, and unwatching turns it off again.
 */
function LiveViewer({ visitor, onClose }: { visitor: PresenceVisitor; onClose: () => void }) {
