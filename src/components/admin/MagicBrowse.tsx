...
import type { LiveVisitor } from '../../lib/adminApi';
...
/**
 * Live session view (MagicBrowse) — Crisp-style. The visitor's host page is
...
export function MagicBrowse({
  ...
  const currentUrl = href || visitor?.url || '';
  const geoText = visitor?.geo ? [visitor.geo.city, visitor.geo.region, visitor.geo.country].filter(Boolean).join(', ') : '';
